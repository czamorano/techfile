...
@Component({
    selector: 'jhi-pensionista-delete-dialog',
    templateUrl: './pensionista-delete-dialog.component.html'
})
export class PensionistaDeleteDialogComponent {
...
@Component({
    selector: 'jhi-pensionista-delete-popup',
    template: ''
})
export class PensionistaDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ pensionista }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(PensionistaDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
